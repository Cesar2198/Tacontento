...
namespace XShop.GUI
{
    public partial class PreRepMejorVenForm : Form
    {
        ...
        public void CargarRegistros(int anio,int numeromes)
        {
            try
            {
                _DATOS.DataSource = CacheManager.CLS.Cache.MejorVendedor(anio,numeromes);
                FiltrarLocalmente();
            }
            catch (Exception)
            {
        ...
        private void btnVer_Click(object sender, EventArgs e)
        {
            ...
            Reportes.GUI.ReporteMejorVendedorForm f = new Reportes.GUI.ReporteMejorVendedorForm(this.anio, item.Key,CLS.Utility.DevolverNombreMes(item.Key));
            f.ShowDialog();
        }
