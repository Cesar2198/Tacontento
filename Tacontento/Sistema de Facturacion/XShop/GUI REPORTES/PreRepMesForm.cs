...
namespace XShop.GUI
{
    public partial class PreRepMesForm : Form
    {
        ...
        public void CargarRegistros(int anio)
        {
            try
            {
                _DATOS.DataSource = CacheManager.CLS.Cache.VentasPorMes(anio);
                FiltrarLocalmente();
            }
            catch (Exception)
            {
        ...
        {
            Reportes.GUI.ReporteVentasMesForm f = new Reportes.GUI.ReporteVentasMesForm(int.Parse(this.txbanio.Text));
            f.ShowDialog();
        }
